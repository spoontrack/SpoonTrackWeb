...
if (isset($_POST['singup-submit'])) {
	...
	if (empty($username) || empty($email) || empty($password) || empty($passwordRepeat)){
		header("Location: ../singup.php?error=emptyfields&uid=".$username."&mail=".$email);
		exit();
	}
		...
		if (!mysqli_stmt_prepare($stmt, $sql)) {
			...
			if ($resultCheck > 0) {
				header("Location: ../singup.php?error=usertaken&mail=".$email);
				exit();
			}
			else {
				...
				if (!mysqli_stmt_prepare($stmt, $sql)) {
					...
					mysqli_stmt_execute($stmt);
					header("Location: ../singup.php?singup=success");
					exit();
